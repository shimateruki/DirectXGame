...
void BossCore::Initialize(Object3dCommon* common, const std::string& modelName) {
    ...
    for (int i = 1; i <= 20; ++i) {
        CorePiece piece;
        piece.obj = std::make_unique<Object3d>();

        // commonを使って初期化（エンジンの仕様に合わせてください）
        piece.obj->Initialize(common);

        // ★ エクスポートした破片のモデル名に合わせてください（例: "core_broken_1" など）
        piece.obj->SetModel("core_piece_" + std::to_string(i));

        // 最初は見えないように隠しておく
        piece.obj->SetScale({ 0.0f, 0.0f, 0.0f });
        piece.obj->SetCollisionAttribute(0);

        corePieces_.push_back(std::move(piece));
    }
}
