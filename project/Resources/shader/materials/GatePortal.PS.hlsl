...
float4 main(VSOutput input) : SV_TARGET
{
    ...
    float warmSpiral = saturate(
        ...
        fineFilaments * 0.28f +
        petalRibbons * 0.42f +
        bakedLine * portalMask * 0.22f);
    float orangeStripe = pow(saturate(0.50f + 0.50f * sin(angle * 4.8f + radius * 13.2f - t * 2.05f + coarse * 2.8f)), 2.25f) * portalMask;
    float yellowStripe = pow(saturate(0.50f + 0.50f * sin(angle * -3.6f + radius * 10.4f + t * 1.55f + midNoise * 2.4f)), 3.20f) * portalMask;
    ...
    if (mode > 1.5f)
    {
        goldDeep = float3(0.360f, 0.150f, 0.035f);
        goldMid = float3(0.900f, 0.405f, 0.075f);
        goldHot = float3(1.000f, 0.780f, 0.330f);
        coreDeep = float3(0.105f, 0.028f, 0.006f);
        coreMid = float3(0.790f, 0.250f, 0.040f);
        coreHot = float3(1.000f, 0.760f, 0.350f);
    }
    ...
    float3 pearlColor = float3(1.000f, 0.895f, 0.570f);
    float3 veilColor = float3(1.000f, 0.560f, 0.175f);
    float3 spaceColor = lerp(float3(0.145f, 0.040f, 0.010f), float3(0.940f, 0.315f, 0.045f), saturate(spaceNebula * 0.48f + distantNebula * 0.28f + corePulse * 0.10f));
    spaceColor = lerp(spaceColor, float3(0.060f, 0.014f, 0.004f), eventHorizon * 0.26f);
    spaceColor = lerp(spaceColor, float3(1.000f, 0.765f, 0.330f), saturate(distantNebula * 0.22f + tunnelRings * 0.14f + yellowStripe * 0.16f));
    spaceColor += float3(1.000f, 0.805f, 0.300f) * starwardCore * 0.12f;
    ...
    innerColor += float3(1.000f, 0.620f, 0.180f) * astralDust * 0.34f;
    innerColor += float3(1.000f, 0.850f, 0.360f) * starwardCore * 0.18f;
    innerColor += pearlColor * petalRibbons * 0.28f;
    innerColor += float3(1.000f, 0.470f, 0.090f) * orangeStripe * 0.20f;
    innerColor += float3(1.000f, 0.900f, 0.360f) * yellowStripe * 0.18f;
    ...
    finalColor += lerp(coreHot, goldHot, 0.38f) * (fineFilaments * 0.06f + sparkInside * 0.11f + glintB * 0.16f + electricWeb * 0.20f + corePulse * 0.10f + petalRibbons * 0.12f + warmSpiral * 0.08f) * intensity;
    ...
    finalColor += float3(1.000f, 0.520f, 0.140f) * astralDust * (0.18f + intensity * 0.10f);
    finalColor += float3(1.000f, 0.830f, 0.350f) * starwardCore * 0.12f * intensity;
    finalColor += float3(1.000f, 0.390f, 0.065f) * orangeStripe * 0.10f * intensity;
    finalColor += float3(1.000f, 0.850f, 0.270f) * yellowStripe * 0.08f * intensity;
