...
float4 main(VSOutput input) : SV_TARGET
{
    ...
    float2 gateMaskUV = saturate(float2(p.x / 0.72f, p.y) * 0.5f + 0.5f);
    float4 bakedSwirl = bakedGateSwirlTex.Sample(smp, gateMaskUV);
    float outerRadius = 0.94f + softness * 0.03f;
    float portal = 1.0f - smoothstep(outerRadius - 0.22f, outerRadius, radius);
    float innerFade = smoothstep(0.03f, 0.38f, radius);
    float edge = RingMask(radius, outerRadius - 0.10f, 0.16f + softness * 0.16f);
    float hotEdge = RingMask(radius, outerRadius - 0.22f, 0.08f + softness * 0.08f);
    portal = saturate(portal * 0.84f + bakedSwirl.a * 0.34f);
    edge = saturate(edge * 0.74f + bakedSwirl.g * 0.48f);
    hotEdge = saturate(hotEdge + bakedSwirl.b * 0.22f);
    ...
    arms = saturate(arms * 0.68f + bakedSwirl.r * 0.52f);
    float inward = pow(saturate(1.0f - radius), 1.6f + depthPower * 0.35f);

    float spark = smoothstep(0.78f, 0.98f, fine + arms * 0.36f + bakedSwirl.g * 0.18f) * portal;
    float flow = saturate(arms * 0.58f + coarse * 0.42f);
    float coreDark = pow(saturate(1.0f - radius * 1.24f), 2.0f + depthPower);
    coreDark = saturate(coreDark + bakedSwirl.b * 0.12f);
