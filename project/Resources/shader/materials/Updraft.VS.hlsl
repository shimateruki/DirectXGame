...
VSOutput main(VSInput input)
{
    ...
    float sway = sin(quad.y * (4.0f + waveFrequency * 0.2f) + time * max(waveSpeed, 0.05f) * 2.0f) * waveHeight * 0.025f;
    float3 worldOffset = viewRight * (quad.x + sway) * proxyRadius + viewUp * quad.y * proxyRadius;
