...
VSOutput main(VSInput input)
{
    ...
    float drift = sin(dot(quad, float2(3.7f, 5.1f)) + time * max(waveSpeed, 0.05f) * 1.4f) * waveHeight * 0.025f;
    float3 worldOffset = viewRight * (quad.x + drift) * proxyRadius + viewUp * (quad.y - drift * 0.6f) * proxyRadius;
