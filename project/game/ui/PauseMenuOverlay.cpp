...
void PauseMenuOverlay::UpdateSprites() {
    const Vector4 normalText = { 0.60f, 0.74f, 0.84f, 0.74f };
    ...
    for (int i = 0; i < static_cast<int>(Item::Count); ++i) {
        ...
        const Vector4 selectedText = { 0.28f + pulse * 0.18f, 0.84f + pulse * 0.10f, 1.0f, 1.0f };
        ...
        if (row.backdrop) {
            ...
            row.backdrop->SetColor(selected ? Vector4{ 0.40f + pulse * 0.10f, 0.90f, 1.0f, 0.98f } : Vector4{ 0.78f, 0.94f, 1.0f, 0.68f });
        }
        if (row.label) {
