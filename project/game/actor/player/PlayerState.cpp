...
// ========================================================
static void FindFeetRecursive(Object3d *node, Object3d *&leftOut,
                              Object3d *&rightOut) {
  if (!node)
    return;
...
// シーン検索（名前ベース）
// ========================================================
static void FindFeetInSceneByName(Player *player, Object3d *&leftOut,
                                  Object3d *&rightOut) {
...
static void TryFindFeet(Player *player, Object3d *&leftOut,
                        Object3d *&rightOut) {
  if (!player)
    return;
...
// 腕探索ヘルパー
// ========================================================
static void FindArmsRecursive(Object3d *node, Object3d *&leftArmOut,
...
static void FindArmsInSceneByName(Player *player, Object3d *&leftArmOut,
                                  Object3d *&rightArmOut) {
  if (!SceneManager::GetInstance())
    return;
  auto scene = SceneManager::GetInstance()->GetCurrentScene();
...
static void TryFindArms(Player *player, Object3d *&leftArmOut,
                        Object3d *&rightArmOut) {
  if (!player)
    return;
...
void PlayerStateAttack1::Enter(Player *player) {
  ...
  MeshEffectManager::GetInstance()->SpawnEffect(
      "Resources/json/effect/effect_attak1.json");
  ...
  if (bodyObj_) {
    ...
    // 変更点: 攻撃ポーズの Y
    // 回転を「現在の向き（bodyDefaultRot_.y）を基準にした相対角度」にする
...
void PlayerStateAttack1::Update(Player *player) {
  ...
  if (animTimer_ >= animDuration_) {
    // 変更点:
    // Attack1 のアニメが終わったとき、Pending
    // フラグ（またはコンボ時間窓）があれば 一度 Idle に戻さず直接 Attack2
    // に遷移するようにする。
    ...
    if (goAttack2) {
      ...
      if (initializedParts_) {
        ...
        if (rightArmObj_) {
          ...
          s_pendingIdleBlend.rightArmStart =
              rightArmObj_->GetTransform()->rotate;
        ...
        if (rightFootObj_) {
          ...
          s_pendingIdleBlend.rightFootStart =
              rightFootObj_->GetTransform()->rotate;
          s_pendingIdleBlend.rightFootTarget = rightFootDefaultRot_;
        }

        if (leftFootObj_) {
          s_pendingIdleBlend.leftFoot = true;
          s_pendingIdleBlend.leftFootStart =
              leftFootObj_->GetTransform()->rotate;
      ...
      // Idle に戻す直前に Pending 補間データを作成する（Exit
      // ではなく遷移決定局所で作る）
      if (initializedParts_) {
        s_pendingIdleBlend.active = true;
        s_pendingIdleBlend.blendDuration = 0.35f;
        ...
        if (bodyObj_) {
          s_pendingIdleBlend.body = true;
          // 修正: 表示中のワールド回転を開始値として使う（Transform->rotate
          // ではなく Player の回転）
          s_pendingIdleBlend.bodyStart = player->GetRotation();
          s_pendingIdleBlend.bodyTarget = bodyDefaultRot_;
        }
...
void PlayerStateAttack1::Exit(Player *player) {
  ...
  // ここでは補間の Pending を作らない。Idle に遷移する直前（Update
  // 内）で限定的に作るようにした。
}
...
void PlayerStateAttack1::ApplyPose(float t) {
  ...
  if (rightArmObj_) {
    Transform *tf = rightArmObj_->GetTransform();
    tf->translate =
        rightArmDefaultPos_ + LerpVec3(rtArmStartPos, rtArmEndPos, t);
  ...
  if (rightFootObj_) {
    Transform *tf = rightFootObj_->GetTransform();
    tf->translate =
        rightFootDefaultPos_ + LerpVec3(rtFootStartPos, rtFootEndPos, t);
...
void PlayerStateAttack2::Enter(Player *player) {
  ...
  animTimer_ = 0.0f;
  MeshEffectManager::GetInstance()->SpawnEffect(
      "Resources/json/effect/effect_attak2.json");
  ...
  // rightArm: 指定された開始位置/回転。ただし要求により回転.x を 0.0
  // にしてからアニメ開始する
  if (rightArmObj_) {
    Transform *tf = rightArmObj_->GetTransform();
    ...
    Vector3 rotRad = Vector3{0.0f, DegToRad(rotDeg.y),
                             DegToRad(rotDeg.z)}; // x = 0 as requested
    tf->rotate = rotRad;
    tf->quaternion = Math::EulerToQuaternion(tf->rotate);
    tf->isQuaternionMaster = true;
...
void PlayerStateAttack2::Update(Player *player) {
  ...
  if ((im && im->IsActionTriggered("Attack"))) {
    player->SetPendingAttack2(true);
    DebugConsole::GetInstance()->AddLog(
        "Attack2: input detected -> pending Attack3 set");
  }
  ...
  if (animTimer_ >= animDuration_) {
    DebugConsole::GetInstance()->AddLog(
        "Attack2: anim finished (animTimer_=" + std::to_string(animTimer_) +
        ", animDuration_=" + std::to_string(animDuration_) + ")");

    // 予約があればAttack3へ
    if (player->ConsumePendingAttack2()) {
      DebugConsole::GetInstance()->AddLog(
          "Attack2: ConsumePendingAttack2() == true -> Change to Attack3");
      player->ChangeState(std::make_unique<PlayerStateAttack3>());
    } else {
      DebugConsole::GetInstance()->AddLog(
          "Attack2: no pending -> prepare s_pendingIdleBlend and Change to "
          "Idle");
      if (initializedParts_) {
        if (!s_pendingIdleBlend.active) {
          ...
          if (leftArmObj_) {
            s_pendingIdleBlend.leftArm = true;
            s_pendingIdleBlend.leftArmStart =
                leftArmObj_->GetTransform()->rotate;
            s_pendingIdleBlend.leftArmTarget = leftArmDefaultRot_;
          }
...
void PlayerStateAttack2::ApplyPose(float t) {
  ...
  Vector3 bodyStartRot = bodyDefaultRot_;
  // 変更点: Y
  // 回転をプレイヤーの向き（bodyDefaultRot_.y）に対する相対値で指定して、向いている方向で攻撃が出るようにする
  float baseY = bodyDefaultRot_.y;
  bodyStartRot.y = baseY + DegToRad(60.0f);

  // Head start: Enterで設定済み
  Vector3 headStartPos{0.0f, 0.0f, 0.0f};
  Vector3 headStartRot =
      headObj_ ? headObj_->GetTransform()->rotate : Vector3{0, 0, 0};

  // Right arm start: Enterで設定済み
  Vector3 rtArmStartPos =
      rightArmObj_ ? rightArmObj_->GetTransform()->translate : Vector3{0, 0, 0};
  Vector3 rtArmStartRot =
      rightArmObj_ ? rightArmObj_->GetTransform()->rotate : Vector3{0, 0, 0};

  // Left arm start: Enter で設定した Attack1 終点相当
  Vector3 ltArmStartPos =
      leftArmObj_ ? leftArmObj_->GetTransform()->translate : Vector3{0, 0, 0};
  Vector3 ltArmStartRot =
      leftArmObj_ ? leftArmObj_->GetTransform()->rotate : Vector3{0, 0, 0};

  // Right foot start
  Vector3 rtFootStartPos = rightFootObj_
                               ? rightFootObj_->GetTransform()->translate
                               : Vector3{0, 0, 0};
  Vector3 rtFootStartRot =
      rightFootObj_ ? rightFootObj_->GetTransform()->rotate : Vector3{0, 0, 0};

  // Left foot start
  Vector3 ltFootStartPos =
      leftFootObj_ ? leftFootObj_->GetTransform()->translate : Vector3{0, 0, 0};
  Vector3 ltFootStartRot =
      leftFootObj_ ? leftFootObj_->GetTransform()->rotate : Vector3{0, 0, 0};
...
void PlayerStateAttack3::Enter(Player *player) {
  ...
  animTimer_ = 0.0f;
  MeshEffectManager::GetInstance()->SpawnEffect(
      "Resources/json/effect/effect_attak3.json");
  ...
  if (headObj_) {
    headDefaultPos_ = headObj_->GetTransform()->translate;
  }
  if (rightArmObj_) {
    rightArmDefaultPos_ = rightArmObj_->GetTransform()->translate;
  }
  ...
  if (s_pendingIdleBlend.active) {
    if (bodyObj_)
      bodyDefaultRot_ = Vector3{0.0f, s_pendingIdleBlend.bodyTarget.y, 0.0f};
    if (headObj_)
      headDefaultRot_ = s_pendingIdleBlend.headTarget;
    if (rightArmObj_)
      rightArmDefaultRot_ = s_pendingIdleBlend.rightArmTarget;
    ...
    if (bodyObj_)
      bodyDefaultRot_ = Vector3{0.0f, frontY, 0.0f};
    if (headObj_)
      headDefaultRot_ = headObj_->GetRotation();
    if (rightArmObj_)
      rightArmDefaultRot_ = rightArmObj_->GetRotation();
  ...
  // 2.
  // ★超重要：Attack2をいじらずに瞬間移動を直すため、Attack2の「最後のポーズ」を手動で開始ポーズにする！
