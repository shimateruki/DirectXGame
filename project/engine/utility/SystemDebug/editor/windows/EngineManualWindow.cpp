    ...
    static const std::vector<ManualPage> pages = {
        {
            ...
            {
                "Hierarchy下部のScene File名を確認し、今から編集するステージと一致しているか確認します。",
        ...
        },
        {
            ...
            "シーン内オブジェクトの一覧、親子関係、表示、編集ロック、検索、カテゴリ、Layerを管理します。上部には全専用Editor、下部にはScene File管理があります。",
            "左ペインの「Hierarchy」タブを開きます。システム設定一覧の下にScene File、検索、分類、Layer、オブジェクトツリーが並びます。",
            {
            ...
            "Hierarchy下部のScene FileからPlayer、Enemy、Object、CameraまたはAllを保存します。親子関係、表示、Transformなどは対象カテゴリJSONへ入ります。",
            {
        ...
        },
        {
            ...
            "Hierarchyの「シーンファイル管理 (Scene File)」を開き、既存ファイルまたは保存名を選びます。",
            {
                "保存名が現在作業中のステージ本体JSONになっているか確認します。_player等の分割ファイルを直接選ぶ画面ではありません。",
                "未保存表示でPlayer、Enemy、Object、CameraのどこがDirtyか確認します。",
                "担当範囲だけなら個別保存を押します。Camera ObjectはCameraのみ保存を使います。",
                "全カテゴリを一式で確定するときだけ「シーン全体保存 (All)」を使います。",
        ...
        },
        {
            ...
            "isolated preview background floor camera speed loop",
            "ゲームシーンから離れた隔離空間で、ParticleとMesh Effectを背景、床、カメラ、速度を揃えて比較します。",
            "Hierarchy最上部の「エフェクト確認ステージ (Effect Preview)」を選びます。",
            {
                "Preview有効化を行い、背景色、床、カメラ距離、再生速度を設定します。",
                "確認するEffectを選び、One ShotまたはLoopで再生します。",
                "正面、斜め、遠距離、近距離から輪郭、Alpha、発光を確認します。",
                "比較するEffectは同じ背景・距離・速度に揃えて差を確認します。",
                "調整後はPreviewを終了し、ゲームシーンの本番位置でも確認します。",
            },
            "Preview Stage自体は保存対象ではありません。調整した各Particle / Mesh Effect Editor側でJSONを保存します。床などの補助ObjectはEditorOnlyです。",
            {
                "隔離空間と本番背景の両方で読める。",
                "再生速度1.0でタイミングが成立する。",
                "Preview終了後にEditorカメラと発生位置が通常状態へ戻る。",
            },
            "隔離プレビューは調整しやすい反面、本番のPost Effect、Light、遮蔽物を再現しません。ここだけで完成判定しないでください。",
        },
        {
            "03 VFX",
        ...
        },
        {
            ...
            "cinematic multi object camera VFX timeline auto key sequence path preview 軌跡",
            "複数Object、Camera Object、VFXを一つのタイムラインで同期し、ムービーやステージクリア演出を制作します。",
        ...
        },
        {
            ...
            "build package Debug Development Release ExecutableSets",
            "選択ConfigurationをBuildし、実行ファイル、DLL、Resourcesを配布確認用フォルダへまとめます。",
            "Hierarchy最上部の「実行ファイルセット (Executable Package)」を選びます。",
            {
                ...
                "進捗とログを確認し、EXE、DLL、Resourcesのコピー完了を待ちます。",
                "作成されたフォルダからEXEを直接起動し、シーン、Audio、Font、Shader、JSONを確認します。",
                "提出前はReleaseでEditor非表示と操作・終了を確認します。",
            },
            "通常はプロジェクトと同階層のExecutableSets/<set name>へ作成します。.cache、.backup、.trash、toolsは除外されます。",
            {
                "作成ログにBuild / Copy Errorがない。",
                "開発環境の作業フォルダに依存せず単体起動できる。",
                "必要な生成Assetと最新JSONが含まれる。",
            },
            "同名Setフォルダは作り直し時に削除・再生成されます。手作業で置いたファイルを同フォルダへ保存しないでください。",
        },
        {
            "07 運用・データ",
