...
void DrawShortcutTable() {
    ...
    AddShortcutRow("左ダブルクリック / Tab", "Game View上の作成パレットを開きます。右ドラッグのカメラ操作では開きません。");
    ...
    AddShortcutRow("Ctrl + K", "操作、Object、Asset、Scene、Preset、Editorウィンドウを横断する統合検索を開きます。");
    AddShortcutRow("Ctrl + Shift + 1～9 / Ctrl + 1～9", "現在のEditorカメラ地点を保存 / 保存済み地点へ復帰します。");
    ...
    static const std::vector<ManualPage> pages = {
        ...
        },
        {
            ...
            "通常のEditor画面中央がGame Viewです。上部メニューの「表示」からHierarchy / Inspectorも表示して併用します。",
            {
                ...
                "Game Viewの空き位置で左ダブルクリック、またはTabを押すと作成パレットが開きます。右ドラッグはカメラ専用なのでメニューは開きません。",
        ...
        },
        {
            ...
            "Game Viewまたは対応するEditorを操作中に使います。文字入力欄へフォーカスしている間は、入力欄の操作が優先される場合があります。",
            {
                ...
                "配置作業では左ダブルクリックまたはTabで作成パレットを開き、左クリックで確定、右クリックまたはEでキャンセルします。",
        ...
        },
        {
            "00 基本",
            "統合検索 / シーン編集ワークスペース",
            "Ctrl K command palette quick search workspace isolate layer bookmark layout Object Asset Scene Preset 統合検索 隔離表示",
            "目的の操作やAssetへ素早く移動し、作業内容に合わせた画面配置、一時表示、Layer表示、Editorカメラ地点を管理します。Scene JSONを変更せずに広いシーンを安全に編集するための補助機能です。",
            "Ctrl+Kで統合コマンドパレットを開きます。画面配置は上部の「ワークスペース」、一時表示とカメラ地点はHierarchyの「シーン編集ワークスペース」を使います。",
            {
                "Ctrl+Kを押し、操作名、Object名、Asset名、Scene、Preset、Editor名の一部を入力します。上下キーで選び、Enterまたは左ダブルクリックで実行します。",
                "よく使う結果は星を付けます。Model、Preset、Prefabの検索結果は配置プレビューを開始し、それ以外のAssetはPathをクリップボードへコピーします。",
                "上部のワークスペースから地形、VFX、アニメーション、Replayを選び、作業用ウィンドウ配置へ切り替えます。調整後は「現在の配置をこのワークスペースへ保存」で更新できます。",
                "Hierarchyの「選択のみ表示」「選択を隠す」「Layer一時表示」で必要なObjectだけを見やすくします。「全表示を復元」で必ず戻せます。これらはScene保存対象ではありません。",
                "Ctrl+Shift+1～9で現在のEditorカメラ地点を保存し、Ctrl+1～9で復帰します。Hierarchyのカメラブックマーク欄から保存・復帰・削除もできます。",
            },
            "お気に入りはoutput/editor_state/editor_quick_search.json、カメラ地点はoutput/editor_state/scene_workspace_bookmarks.json、ワークスペース配置はoutput/editor_state/workspacesへ保存されます。SceneのObject JSONには入りません。",
            {
                "統合検索で選んだObjectとHierarchy / Inspectorの選択が一致する。",
                "一時非表示を使った後に「全表示を復元」し、見落としたObjectがない。",
                "ワークスペース保存前に、意図しないウィンドウやモーダルを開いたままにしていない。",
                "カメラブックマークが対象Sceneごとに正しい地点へ戻る。",
            },
            "一時非表示はSceneのVisible設定とは別です。見えないObjectを削除済みと判断せず、全表示を復元してから保存・監査してください。",
        },
        {
            "01 シーン編集",
            "Play In Editor / 変更の持ち帰り",
            "PIE Play Stop Ctrl P runtime change tracker 持ち帰り Transform 設定値 Undo",
            "現在の編集Sceneをメモリ上で試遊し、停止時に元の編集状態へ戻します。Play中に調整した編集可能ObjectのTransformや設定値だけを選んで持ち帰ることもできます。",
            "上部の再生ボタン、またはCtrl+PでPlayを開始・停止します。",
            {
                "Play前にActive Sceneと未保存表示を確認します。開始時のObject・Camera・Scene状態が復元用に記録されます。",
                "ゲームを操作し、当たり判定、移動、Camera、VFX、Audio、ゴールまで確認します。Play中のPlayer・EnemyのHPや速度は実行時状態として扱われます。",
                "停止すると開始時の編集Sceneへ戻ります。Play中に変更された編集可能Objectがある場合だけ「Play中変更の持ち帰り」が自動で開きます。",
                "Objectごとに「Transformを反映」「設定値を反映」を選び、「選択した変更を反映」を押します。不要なら「すべて破棄」を押します。",
                "持ち帰りはUndo可能です。Game ViewとInspectorで確認し、必要なカテゴリだけScene Fileから保存します。",
            },
            "Play中の状態は自動保存されません。持ち帰りを適用した後、Scene AssetのPlayer、Enemy、Object、Cameraなど該当カテゴリへ明示保存します。",
            {
                "停止後にPlay前のSceneとCameraへ戻った。",
                "Player・Enemyの一時的なHPやAI状態を編集値として持ち帰っていない。",
                "動く床など実行時駆動Gimmickの移動結果を配置値として誤反映していない。",
                "持ち帰り後にCtrl+Zで戻せ、再Playでも意図した挙動になる。",
            },
            "Play中の見た目をそのまま保存する機能ではありません。実行時に自然に変化した値と、調整目的で変えた値を区別し、必要な項目だけ持ち帰ってください。",
        },
        {
            "01 シーン編集",
            "Hierarchy",
            ...
            "左ペインの「Hierarchy」タブを開きます。システム設定一覧の下にScene Asset、検索、分類、Layer、オブジェクトツリーが並びます。",
            {
                ...
                "Hierarchyの空白を右クリックして作成メニューを開くか、モデル・Presetをドラッグして新規オブジェクトを作成します。Game View側は左ダブルクリックまたはTabです。",
        ...
        },
        {
            ...
            "Hierarchyの「Scene Asset」を開くか、上部メニューの「ファイル」から「新規Scene Asset」を選びます。作成済みAssetは上部の「シーン切り替え」にも自動表示されます。",
            {
                ...
                "ゲーム実行用にはTITLE、GAME、GAME_OVERを選びます。Scene Assetの編集専用で開く場合はSCENE_EDITORを使用します。新しいゲームはTITLEを起点にします。",
                ...
                "検証ボタンで重複Object名、親子参照、リソースパスなどを確認します。エラーがあるAssetは開く前に修正します。",
                ...
                "共通描画処理はRuntimeSceneへ集約されています。TITLE、GAME、GAME_OVER固有の処理は各SceneのInitializeSceneContents、または用途ごとのComponentへ分離します。",
            },
            "ObjectはResources/json/3Dobject/<id>.jsonとカテゴリ別JSON、SpriteはResources/json/sprite/<id>_sprite.jsonへ保存されます。実行クラスとControllerは_sceneAsset、各リソースは_sceneAsset.resourcesへ保存されます。",
            {
                ...
                "TITLE、GAME、GAME_OVERで空Sceneを開いたときもCamera、Light、Skybox、Editor操作が利用できる。",
            ...
            "SceneLoadContextはScene初期化前に注入されます。編集専用AssetにはSCENE_EDITOR、ゲーム実行用AssetにはTITLE、GAME、GAME_OVER、または新しく登録したSceneを使用してください。",
        },
        {
            "01 シーン編集",
            "Scene File / シーン保存",
        ...
        },
        {
            "01 シーン編集",
            "Inspector / Material",
            "Transform Collision PBR Material Instance Surface Decal Gimmick Enemy Item Camera LOD マテリアルインスタンス デカール",
            ...
            "HierarchyまたはGame Viewでオブジェクトを選択し、右ペインの「Inspector」を開きます。",
            {
                ...
                "同じ見た目を共有する場合はMaterial Instanceで既存Assetを選びます。現在値を新しいInstanceへ保存、再読込、リンク解除もここから行えます。",
                "Surface DecalではRendererで画像とBlendを設定し、専用欄でサイズ、面オフセット、寿命、Fade In / Outを調整して再生確認します。",
                "下部のEnemy、Gimmick、Item、Spawner、Particle、Ghost Recorder、LODなどクラス固有項目を設定します。",
            },
            "Object設定はScene Fileの対象カテゴリ、Material InstanceはResources/json/material_instances/<name>.jsonへ保存します。DecalはObjectとしてScene JSONへ入り、元画像自体は書き換えません。",
            {
                "モデル変更後にCollisionとスケールを再確認した。",
                "透明物のBlend、影、描画順が本番カメラで正しい。",
                "Material Instanceを再読込して、親Asset継承とリンク中Objectの見た目が維持される。",
                "Decalが地面・壁から浮かず、寿命終了後に残らず、極端な角度で不自然に伸びていない。",
        ...
        },
        {
            "02 描画・入力",
            "カメラ設定 (Camera)",
            "Camera Editor Main Cinematic Camera Object preview follow easing FOV Mixer shake impulse Feedback Cue",
            "Editor自由カメラとゲーム用カメラ設定を扱い、Camera Objectの演出切替と、複数のShake・FOV Pulseを合成するCamera Mixerを確認します。",
            "Hierarchy最上部の「カメラ設定 (Camera)」を選びます。演出用カメラは作成メニューからCamera Objectを生成し、そのObjectを選択して専用Inspectorを使います。",
            {
                ...
                "爆発や被弾のCamera Shake / FOV PulseはVFX SequencerのFeedback Cueから試します。複数Cueが重なっても後の揺れで前の揺れが消えないことを確認します。",
            ...
            "カメラ設定はResources/json/camera/<name>.json、Editor自由カメラ状態はResources/json/camera/editor_camera_state.json、Camera ObjectはScene FileのCameraカテゴリへ保存します。",
            {
                ...
                "連続する被弾・爆発でShakeとFOVが急に途切れず、演出終了後に基準FOVへ戻る。",
            },
        ...
        },
        {
            "02 描画・入力",
            "ライティング設定 (Lighting)",
            "Light Directional Point Spot Fog Skybox IBL Environment Profile Bloom Tone Mapping LUT Gameplay Volume",
            "Directional、Point、Spot、Fog、Skybox、環境光を調整し、Environment ProfileでLight・Post Effect・色調補正を一括保存して滑らかに切り替えます。",
            "Hierarchy最上部の「ライティング設定 (Lighting)」を選びます。",
            {
                ...
                "Environment Profile欄でファイル名と切替時間を設定し、「現在の環境を保存」でLight、Fog、Skybox、Shadow、Bloom、Tone Mapping、LUTを一式保存します。",
                "「Profileを適用」で補間を確認します。エリア連動はGameplay Volumeの処理をEnvironment Profileにし、Profile PathとBlend時間を設定します。",
            },
            "個別LightはResources/json/light/<name>.json、Environment ProfileはResources/json/environment/<name>.jsonへ保存します。Gameplay Volume側の参照はScene Object JSONへ入ります。",
            {
                "影が黒つぶれせず、発光物も白飛びしていない。",
                "Point / Spotの個数と範囲が必要最小限である。",
                "別カメラ角度でもライトが急に消えたりモデル裏面だけ暗くならない。",
                "Profile切替中にLightだけでなくBloom、Tone Mapping、Fogも同じ時間で遷移し、退出時に元環境へ戻る。",
            },
        ...
        },
        {
            ...
            "比較的軽量で少数の粒子表現を作り、Shape、物理、色、寿命に加えて、共通Curve / Gradientと距離LODをJSONとして調整します。",
            "Hierarchy最上部の「通常パーティクル (Particle)」を選びます。",
            {
                ...
                "既存Presetと同じ見た目を保つ場合は従来の10点Size Graphを使います。細かいキー編集が必要な時だけAuthoring Curveを有効にします。",
                "Authoring CurveではTime、Value、区間Easingを調整し、Gradientでは発生から消滅までの色とAlphaのキーを編集します。ドラッグ1回はCtrl+Zで1回の操作として戻せます。",
                "距離LODを使う場合は近距離・遠距離・遠距離発生率を決め、同時生存上限でCPU Particleの最大数を抑えます。",
                "単発とループを確認し、画面外や終了後に粒子が残らない状態で保存します。",
            },
            "Resources/json/particle/<name>.jsonへ保存します。保存したJSONはProjectや対応するObject設定から参照します。",
            {
                "寿命と発生間隔から同時粒子数が増えすぎず、LOD有効時は距離と生存上限が意図どおり効く。",
                "Alphaが0へ戻り、ループ停止後に消える。",
                "ゲーム本番のカメラ距離と背景色でも形が読める。",
            },
            "Authoring CurveとLODは初期状態で無効なので、既存JSONの見た目は従来の10点Curveのままです。大量発生や広範囲演出はGPU Particleとの使い分けを検討してください。",
        },
        {
            "03 VFX",
            "GPUパーティクル (GPU Particle)",
            "GPU Particle burst aura fire smoke collision spritesheet quick preset",
            "大量粒子、爆発、火花、煙、オーラ、収集演出をGPUで描画し、共通Curve / Gradient、距離LOD、衝突、環境影響、Spritesheetを調整します。",
            "Hierarchy最上部の「GPUパーティクル (GPU Particle)」を選びます。",
            {
                ...
                "Color、Alpha、Size、回転、Blend、Spritesheetを調整します。共通Curve / GradientはGPU互換の開始・中間・終了3点を編集します。",
                "距離LODでは近距離から遠距離へ発生率を落とし、必要なら同時生存上限を設定します。GPU Systemの最小容量に合わせ、0以外は256以上になります。",
            ...
            "Resources/json/gpu_particles/<name>.jsonへ保存します。VFX Sequencer、Trail Emitter、GPUParticle Objectから参照できます。",
            {
                ...
                "発生数、寿命、間隔に対してSystem実行容量が過剰でも不足でもなく、LOD上限が実行容量にも反映される。",
        ...
        },
        {
            ...
            "timeline Feedback Cue GPU Particle Mesh Effect Audio Event camera shake FOV hit stop rumble light sequence",
            "GPU Particle、Trail、Mesh Effect、Audio、Lightに加え、Camera Shake、FOV Pulse、Hit Stop、Controller Rumbleを同じ時間軸へまとめるGameplay Feedback Cueです。",
            "Hierarchy最上部の「VFXシーケンサー (VFX Sequencer)」を選びます。",
            {
                ...
                "Hit Stopは停止時間と強さ、Rumbleは左右モーターと持続時間、FOV Pulseは変化量・立上り・保持・戻りを設定します。",
                "Camera ShakeとFOV PulseはCamera Mixerで加算されます。重なる攻撃を連続再生し、一方が他方を消さないことを確認します。",
                "SEのバリエーションが必要な場合は直接音声PathではなくAudio Event JSONを指定します。Animation EventからはEvent名FeedbackCue、PayloadにこのSequence名を設定して発火できます。",
                "Preview Environmentとゲーム内ターゲット位置の両方で確認して保存します。",
                "Animation Eventから発火しない、参照名が違う、途中で止まる場合は、ヘルプのGameplay Event Traceを開いてRequest、Start、Fire、Complete / Failedの順を確認します。",
            },
            "Resources/json/vfx_sequence/<name>.jsonへ保存します。Ghost DirectorのVFXトラックからも参照できます。",
            {
                ...
                "Hit Stop中も入力・演出終了が破綻せず、Controller Rumbleが指定時間で止まる。",
                "複数Cueを連続再生してShake、FOV、Time Scaleが基準値へ戻る。",
            },
        ...
        },
        {
            ...
            "Camera固有値とGimmick / ItemのGameplay値にも対応します。Player / Enemyのゲーム固有Statusを追加した場合は、保存元を一つに決めてMatrixとの重複編集を避けます。",
        },
        {
            "04 データ・検証",
            "プリセットエディタ (Preset Editor)",
            "Preset Prefab v3 Variant Override Apply Revert Unpack palette brush enemy gimmick item template",
        ...
        },
        {
            "04 データ・検証",
            "キャラクターとゲーム機能の拡張口",
            "Character Motor Factory Enemy Gimmick Item register extension",
            "基本移動と衝突はCharacter / CharacterMotorに残し、敵・ギミック・アイテムはゲーム側からFactoryへ登録して追加します。",
            "Playerの基本操作はStarter Sceneで確認し、登録済み型はHierarchyの作成メニューとInspectorに自動表示されます。",
            {
                "PlayerでWASD移動、Spaceジャンプ、坂・段差・壁際・接地吸着を確認します。",
                "新しい敵はBaseEnemy、新しいギミックはBaseGimmick、新しいアイテムはBaseItemを継承します。",
                "各実装の生成関数を対応するFactoryへ登録します。固定メニューの編集は不要です。",
                "登録後にHierarchyとInspectorへ型名が表示され、Scene JSONへ型名が保存されることを確認します。",
                "ゲーム固有の能力値、攻撃、進行、セーブ、イベントはゲーム側のシステムとして追加します。",
            },
            "Factoryはコード登録式です。Scene Objectの型名と追加パラメータはScene JSONへ保存できます。",
            {
                "未登録型をSceneへ保存していない。",
                "Factoryが返すObjectのModel、Collision、初期値が自己完結している。",
                "基本Playerの移動とCamera追従・壁めり込み回避を壊していない。",
            },
            "整理直後のエンジンには具体的な敵、ギミック、アイテム、攻撃Asset、Gameplay Volumeは同梱していません。必要なゲーム仕様だけを追加してください。",
        },
        {
            "04 データ・検証",
            "Asset参照エクスプローラー / 配置物集計",
            "Asset Reference Explorer GUID missing dynamic reference rename trash Scene Inventory 配置物集計 model collision",
            "Assetがどこから参照されているか、Missingや動的参照候補があるかを確認し、現在Sceneに何が何個あるかをカテゴリ・種類・モデル・当たり判定別に集計します。",
            "Asset参照は上部の「表示 → Asset参照エクスプローラー」、配置物集計はHierarchy最上部の「配置物集計 (Scene Inventory)」を選びます。Ctrl+Kから名前検索して開くこともできます。",
            {
                "Asset参照エクスプローラーで「参照を再スキャン」を押し、Assetの参照元、選択ObjectのAsset、Missing参照の各Tabを確認します。",
                "確定参照と動的参照候補を分けて確認します。動的参照候補があるAssetを未使用と断定しないでください。",
                "名前変更は「GUIDを維持して名前変更」を使い、Path参照が残る場合は一覧で修正します。削除は参照件数を確認し、必要な場合だけゴミ箱へ移動します。",
                "Missing参照は近い候補を選び、参照元JSONが変わっていないことを再スキャンで確認してから1件ずつ修復します。",
                "配置物集計では検索と「非表示Objectを含める」を設定し、カテゴリ別、種類別、モデル別、当たり判定Tabで過不足を確認します。",
                "一覧の対象へカメラ移動し、必要なら集計をクリップボードへコピーしてステージ設計や負荷確認に使います。",
            },
            "配置物集計は読み取り専用です。Asset参照の修復は参照元JSON、名前変更はAssetと.meta、削除はゴミ箱移動を伴います。変更後は対象EditorとSceneを再読込して確認します。",
            {
                "重要Assetに確定参照または動的参照候補が表示される。",
                "Missing修復後に参照元Editorで同じAssetを読み込める。",
                "同じモデルやCollisionなしObjectが意図せず大量配置されていない。",
                "非表示の出現待ちGimmickやEditor補助Objectを集計条件で取り違えていない。",
            },
            "動的に文字列を組み立てるPathは完全に追跡できない場合があります。参照0件だけを理由に削除せず、Asset Audit、全Scene、実ゲームを合わせて確認してください。",
        },
        {
            "04 データ・検証",
            "シーン検証 (Scene Validator)",
            "validate visual audit dependency prewarm duplicate Event ID missing target model collision texture safe fix report screenshot",
            "現在シーンの参照・配置・見た目を監査し、安全な修正候補、Game View証拠画像、依存Asset一覧、Prewarm時のMissingをまとめて確認します。",
            "Hierarchy最上部の「シーン検証 (Scene Validator)」を選びます。",
            {
                "「再チェック」を押すか必要な間だけ自動チェックを有効にし、床へのめり込み、OBB重なり、動的経路、収集物の向き、Collider適合など必要な監査項目を選びます。",
                "Error、Warning、Infoで絞り、一覧の対象を選んで「選択問題へ移動」とScene上の赤・シアン表示で原因を確認します。必要なら「選択問題を撮影」で証拠画像を残します。",
                "修正候補のBefore / Afterを確認し、「安全候補だけ選択」または個別選択後に「選択候補を適用」を押します。一括適用はCtrl+Zで戻せます。",
                "配置意図へ影響する要確認候補は自動選択されません。候補を選ぶ前にInspectorとGame Viewで意図を判断します。",
                "Asset Dependency / Prewarmで「依存関係を収集してPrewarm確認」を押し、JSON、Model、Texture、Audio、VFX、Animator、CinematicとMissing Pathを確認します。",
                "「シーン視覚監査を出力」でreport.json、overview.svg、game_view.pngをまとめ、再チェックで問題数が減ったことを確認します。",
                "シーン保存前にErrorとMissingを0にし、実際のSceneロードでも初回VFX・Audioが引っ掛からないか確認します。",
            },
            "監査だけではSceneを保存しません。安全候補の適用はメモリ上のObjectを変更するため、確認後にScene Fileから対象カテゴリを保存します。監査レポートはcaptures/scene_audit/<timestamp>へ出力されます。",
            {
                "Duplicate Event IDとMissing Targetを優先して解消した。",
                "Missing Model / Textureが実行時の動的生成用ではないか確認した。",
                "Prewarm一覧にAudio Event、Animator、Cinematic、VFXの参照先まで含まれている。",
                "安全修正の適用後にCtrl+Zと再適用が成立し、意図しないObjectまで変わっていない。",
                "修正後に再Scanと実ゲームロードを行った。",
            },
            "安全候補も配置意図を保証するものではありません。特に重なり、回転、Collider、Missing参照の候補はBefore / Afterと対象Objectを確認し、変更後にScene保存・再読込・実ゲーム確認を行ってください。",
        },
        {
            "04 データ・検証",
        ...
        },
        {
            ...
            "Animation body transform bone pose keyframe event marker player enemy preview",
            "単体ObjectのBody Transform Animationと、ボーン付きモデルのPose Key、Bone Transform、Event Markerをゲーム挙動から切り離して編集・確認します。",
            "Hierarchy最上部の「アニメーション制作 (Animation Workbench)」を選びます。",
            {
                ...
                "スライムなどの本体変形はBody Transformを選び、Position Offset、Rotation Offset、Scale Multiplierへキーを登録します。",
                "Game ViewのBody Gizmoまたは数値入力で姿勢を作り、キーごとのEasingを選択します。",
            ...
            "AuthoringデータはResources/json/animation_clip/<name>.jsonへ保存します。元モデルのAnimationを直接上書きせず、BodyとBoneの追加データを同じClip Assetとして管理します。",
            {
                "キー間でBoneが最短方向に補間され、不自然な反転がない。",
                "Loop境界で姿勢が跳ねない。",
                "Event Markerが見た目の接触フレームと一致する。",
            },
            "Animation Workbenchは単体キャラクターのBody / Bone編集用です。物理移動はゲーム側へ残し、複数ObjectとCameraを同期するムービーにはGhost Directorを使ってください。",
        },
        {
            "05 アニメーション",
            "Animator Controller",
            "animator state transition parameter trigger crossfade blend controller Animation Event Track Feedback Cue payload normalized time",
            "Playerや敵のState遷移を管理し、Animation Event Trackで足音、攻撃判定、VFX、SEなどをAnimationの正規化時刻へ結び付けます。",
            ...
            {
                "新規Controllerを作成し、StatesでState名、Model内Clip名、Body Animation Clip、速度、Loop、標準Blend時間とEasingを設定します。",
                "PlayerスライムはBody Animation ClipへWorkbench Assetを割り当てます。未設定Stateだけ従来のコードAnimationへフォールバックします。",
                ...
                "StateのEvent TrackへEvent名、Payload、0～1の正規化時刻を追加します。Clip速度を変えても同じ動作フレームへ追従する位置に置きます。",
                "演出を鳴らす場合はEvent名をFeedbackCue、PayloadをVFX Sequence名にします。ゲームロジック用Eventは受信側コードが対応するEvent名だけを使用します。",
                "State開始時、Loop境界、CrossFade途中をPreviewと実ゲームで確認し、0秒Eventやループ先頭が二重発火しないことを確認します。",
            ...
            "Resources/json/animator/<name>.jsonへ保存します。Objectへの割り当てはScene JSONのanimation.animatorControllerへ保存され、ゲーム読込、複製、リプレイ復元でも維持されます。",
            {
                ...
                "Animation Eventが見た目の接触フレームで一度だけ発火し、LoopとCrossFadeで欠落・二重発火しない。",
                "FeedbackCueのPayloadが存在し、VFX・SE・Shake・Hit Stopが同じタイミングで開始する。",
            },
        ...
        },
        {
            ...
            "replay rewind pause playback branch archive input delta regression test screenshot CPU GPU report 自動回帰 デバッグ 巻き戻し",
            "実行中SceneをTime Machineとして巻き戻すだけでなく、入力とdeltaTimeを保存Replayへ記録し、同じゲーム処理を再実行して自動回帰テストできます。",
            "ゲームを再生し、上部の「リプレイ」メニューまたは「表示」から下段Replay Editorを開きます。Editorは通常は非表示で、開いている間だけProject、Console、ステータス領域を置き換える固定パネルになります。自動記録が有効ならシーン遷移完了後に記録が始まります。",
            {
                ...
                "選択時点から別の入力を試す場合は「この時点からプレイ再開」を押します。選択位置より後の履歴は破棄され、新しい時間軸として記録されます。",
                "再現用データを残す場合は現行履歴を保存します。新形式ReplayはObject状態に加えてキーボード、マウス、ゲームパッド入力とsimulation deltaTimeをSaved/Replaysへ保存します。",
                "保存Replayを選び「自動回帰テスト」を押します。元Sceneと初期状態を復元し、記録入力でゲーム処理を再実行します。旧形式Replayは入力列がないため、新しく記録し直してください。",
                "許容座標誤差、ワールド範囲、チェックポイント画像数を設定します。CPU / GPU Budgetで失敗させる場合だけ性能Budget判定を有効にします。",
                "完了後にPASS / FAIL、最大座標誤差、ゴール到達、Errorログ、CPU / GPU時間、画像を確認し、Saved/ReplayRegression/<replay>_<timestamp>/report.jsonを開きます。",
                "通常の編集状態へ戻す場合は上部の停止ボタンを使います。下段Editorを閉じるとProject、Console、ステータスは元の表示へ戻ります。",
            },
            "現在履歴はメモリ上、保存ReplayはSaved/Replays/*.cgr、自動回帰のreport.jsonとGame View画像はSaved/ReplayRegression配下へ保存されます。Scene Fileは書き換えません。",
            {
                "停止中に敵やPlayerの位置、HP、速度、Camera構図が記録時点へ戻る。",
                "分岐再開後に敵、Player、Collisionが動き、新しい履歴と入力列が増える。",
                "自動回帰で基準Replayと同じゴール結果になり、座標誤差が許容値内である。",
                "テスト中の新規Errorが0で、チェックポイント画像が本番Cameraの期待する見た目になっている。",
                "Character MotorやCamera変更後に複数ステージの基準ReplayがPASSする。",
            },
            "自動回帰は入力と保存時刻を再現しますが、外部時刻、乱数、非同期処理、未対応の敵固有私有状態まで完全決定論にはしません。FAIL時はreportだけで判断せず、Timelineと画像で最初にずれた時点を確認してください。",
        },
        {
            "06 生成・最適化",
            "テキストPNG生成 (Text PNG)",
        ...
        },
        {
            ...
            "status extension Character Motor Player Enemy Factory",
            "ゲーム固有ステータスを後から接続するための拡張口です。整理直後は共通ステータス定義を持ちません。",
            "Hierarchy最上部の「ステータス管理 (Status Management)」を選びます。",
            {
                "未登録状態では案内だけが表示されます。",
                "基本Playerの速度、ジャンプ、重力、CharacterMotor設定はPlayer実装から開始できます。",
                "HPや攻撃力などが必要になったらゲーム側にデータ定義と保存先を追加します。",
                "Editorで調整したい値だけをこのウィンドウへ接続し、実行中Objectへの反映方法もゲーム側で定義します。",
            },
            "初期状態では専用ステータスJSONを使用しません。保存形式はゲーム仕様を決めてから追加します。",
            {
                "エンジン共通移動とゲーム固有能力値を分離している。",
                "未登録の敵タイプを固定一覧として表示していない。",
                "Editor変更の保存先と実行時の反映元が一つに決まっている。",
            },
            "旧ゲームのstatus_settings.jsonと敵タイプ別設定は削除済みです。必要な項目だけを新しいゲーム向けに設計してください。",
        },
        {
            "07 運用・データ",
        ...
        },
        {
            "07 運用・データ",
            "音声設定 (Audio Settings)",
            "BGM SE volume loop path preview Audio Event random clips pitch max instances spatial distance attenuation user config",
            "全体BGM / SEと単体Audio Entryに加え、複数Clipのランダム再生、音量・Pitch揺らぎ、同時発音数、3D距離減衰をAudio Eventとして編集します。",
            "Hierarchy最上部の「音声設定 (Audio Settings)」を選びます。",
            {
                ...
                "Audio Event欄でEventファイル名を入力し、候補Clipを複数追加します。音量範囲、Pitch範囲、最大同時発音数を設定します。",
                "位置に応じて減衰させる場合は3D距離減衰を有効にし、減衰開始距離と無音距離を設定します。",
                "保存してから試聴し、連続再生でClipの偏り、機械的な同一音、音割れ、同時発音上限を確認します。",
                "VFX SequencerからAudio Event JSONを指定し、実際の発生位置とCamera距離で聞こえ方を確認します。ゲーム側からBGMトリガーを追加する場合はAudio EntryのIDまたはPathを指定します。",
                "保存後に再読み込みし、ゲーム内の実際の発音タイミングで確認します。",
            },
            "全体設定はResources/json/user_config.json、Audio EntryはResources/json/audio/audio_settings.json、Audio EventはResources/json/audio_events/<name>.jsonへ保存されます。",
            {
                "Audio IDが重複せず、参照先Fileが存在する。",
                "SEを連続再生して音割れ・過大音量にならない。",
                "Loop BGMの終端にクリック音や長い無音がない。",
                "Audio Eventの全Clipが存在し、音量・Pitch範囲のMinがMax以下である。",
                "3D音が近距離で聞こえ、無音距離以遠で止まり、大量発音時も上限を超えない。",
            },
            "現在の3D音響は距離減衰が中心で、左右PanやHRTFによる完全な立体定位ではありません。試聴だけで完成扱いにせず、BGM・複数SE・VFXが重なる本番Cameraで確認してください。",
        },
        {
            "07 運用・データ",
        ...
        },
        {
            "07 運用・データ",
            "内部データ編集 (Game Data)",
            "save progress extension game data",
            "ゲーム固有のセーブや進行データを後から接続するための拡張口です。整理直後は進行データを持ちません。",
            "Hierarchy最上部の「内部データ編集 (Game Data)」を選びます。",
            {
                "未登録状態では案内だけが表示されます。",
                "ステージ進行、所持品、チェックポイントなど必要な仕様を先に決めます。",
                "ゲーム側のデータ管理と保存形式を実装し、必要なDebug操作だけをこの画面へ追加します。",
                "初期化・削除など広い操作には確認UIと検証用データの分離を設けます。",
            },
            "初期状態ではセーブスロットJSONやステージ一覧JSONを使用しません。",
            {
                "Engine設定とゲーム進行データを同じJSONへ混在させていない。",
                "保存データのVersion移行方針がある。",
                "Debug編集が通常プレイ用データを誤って壊さない。",
            },
            "旧ゲームの残機、コイン、王冠、Stage解放、Tutorial進行編集は削除済みです。",
        },
        {
            "07 運用・データ",
            "ゲーム設定 (Game Settings)",
        ...
        },
        {
            ...
            "gameplay event trace Animation Event Feedback Cue VFX sequence Request Start Fire Complete Failed 診断",
            "Animation EventからFeedback Cue、VFX Sequence、個別Particle・Audio・Camera Eventへ渡る流れを時系列で確認し、参照切れや発火順の問題を特定します。",
            "上部メニューの「ヘルプ」から「Gameplay Event Trace」を開きます。ゲーム実行前からCaptureを有効にしておきます。",
            {
                "Clearで以前の記録を消し、Captureが有効な状態で調べたい攻撃、Animation、Sequenceを1回だけ再生します。",
                "RequestでAnimation Event名とPayload、StartでSequence名と対象Object、Fireで個別Event種別とPreset名を確認します。",
                "Completeまで到達したか、Cancel、Warning、Failedで止まっていないかを時刻順に追います。",
                "上部FilterへObject名、Sequence名、Preset名、Event種別の一部を入力し、関係する行だけへ絞ります。",
                "行を選択し、下部の詳細でSource、Asset、Detailを確認して、切れているJSON名や対象Objectを修正します。",
            },
            "この画面は診断用で、SceneやPresetへ保存しません。最大1024件のメモリ内履歴で、Clearまたは再起動により消えます。",
            {
                "Requestの後にStartがなければFeedback Cue名またはSequence JSON参照を確認した。",
                "Startの後にFireがなければTrigger Time、Event数、再生停止条件を確認した。",
                "Warning / FailedのDetailに出た対象名を、Project上の実ファイル名と照合した。",
                "修正後にClearして同じ操作を再実行し、Completeまで到達した。",
            },
            "Traceは原因を記録する診断表示で、演出そのものを修復する機能ではありません。Captureを無効にしていた時間のEventは後から復元できません。",
        },
        {
            "08 補助",
        ...
        },
        {
            ...
            "対象Editorで調整を終えた後、このページを開いて順番に確認します。",
            {
                ...
                "Asset Dependency / Prewarmで全依存AssetとMissingを確認し、Asset参照エクスプローラーでも動的参照候補を確認します。",
                "基準Replayがあるステージは自動回帰テストを実行し、座標、ゴール、Errorログ、CPU / GPU、チェックポイント画像を確認します。",
            },
            "各Editorの保存先に加え、Scene FileのDirty表示がすべて意図した状態になっていることを確認します。",
            {
                ...
                "Buildと単体起動が成功する。",
                "Replay自動回帰のreport.jsonがPASSで、性能Budgetを使う場合はEditor負荷を考慮した基準になっている。",
            },
