...
const std::vector<ManualPage>& GetManualPages() {
    static const std::vector<ManualPage> pages = {
        ...
        {
            "04 データ・検証",
            "ゲームプレイ共通基盤 (Motor / Attack / Volume)",
            "Character Motor Sweep slope step ground Attack Ability Asset Enemy Attack Profile Gameplay Volume checkpoint kill BGM Environment Feedback Cue",
            "キャラクター移動、敵攻撃、ステージ範囲イベントを共通データとして調整します。コードを増やさずに坂・段差・攻撃判定・チェックポイント・落下死・BGM・環境切替を揃えるための基盤です。",
            "Character Motorはステータス管理、攻撃Assetは「敵攻撃プレビュー」、Gameplay VolumeはGame Viewの作成パレットまたはHierarchyの作成メニューから開きます。",
            {
                "Character MotorではPlayerまたはEnemy Typeを選び、「高速移動をSweep判定」を必要な対象だけ有効にします。接地吸着、最大登坂角度、自動段差、接地探索距離、Skin幅を小さく調整します。",
                "坂、段差、壁際、移動床、高速ダッシュ、ノックバックを実ゲームで確認します。従来挙動を維持したい対象は高度なMotor補正をOFFのままにします。",
                "敵攻撃プレビューでEnemy Typeと攻撃種類を選び、判定形状・位置・半径/幅・準備・攻撃継続・後隙・キャンセル受付・吹き飛ばし・無敵時間を設定します。",
                "攻撃中VFXとFeedback Cueを割り当て、予兆、判定発生、被弾、終了までをTimelineと実プレイヤー対象の両方で確認して保存します。",
                "Gameplay Volumeは作成パレットの「ギミック → ゲームプレイボリューム」から配置し、InspectorでBox範囲と処理を選びます。",
                "処理はイベント連携、チェックポイント、落下死、Feedback Cue、BGM、Environment Profileから選びます。進入/退出、一度だけ、再実行間隔、無効化時の復帰も用途に合わせて設定します。",
                "Game Viewのデバッグ表示で範囲を確認し、境界をゆっくり出入りして二重発火、発火漏れ、退出時復元を確認します。",
            },
            "MotorはResources/json/gameplay/status_settings.json、敵攻撃はResources/json/enemy_attack/<enemy>.json、Gameplay Volumeは配置ObjectとしてScene JSONへ保存されます。参照するFeedback Cue、BGM / Audio Entry、Environment Profileは各専用データも必要です。",
            {
                "Motorを有効化した対象が坂や小段差で跳ねず、壁や床をすり抜けない。",
                "攻撃の見た目と当たり判定の開始・終了が一致し、被弾後の無敵時間で多段ヒットしすぎない。",
                "Gameplay Volumeの範囲、Target、Event IDが意図した保存カテゴリとLayerになっている。",
                "BGM・Environment Profileが退出時に元状態へ戻り、One Shot設定時だけ再発火しない。",
            },
            "Motor設定を全キャラクターへ一度に有効化しないでください。攻撃Assetは共通フェーズを扱いますが、敵AI固有の移動や分岐そのものを完全データ化する機能ではありません。代表1体・1攻撃・1Volumeずつ確認して広げてください。",
        },
        {
            "04 データ・検証",
        ...
        },
        {
            ...
            "player enemy stats hp speed gravity model Character Motor Sweep slope step ground snap skin realtime source of truth",
            "PlayerとEnemy Typeの能力値・モデルに加え、Character MotorのSweep衝突、坂、段差、接地吸着を一元管理して現在Sceneへ反映します。",
            "Hierarchy最上部の「ステータス管理 (Status Management)」を選びます。",
            {
                ...
                "Character Motor欄では「高速移動をSweep判定」を必要な対象だけ有効にし、接地吸着、最大登坂角度、自動段差、接地探索距離、Skin幅を調整します。",
                "Motor補正を有効にしたら、平地だけでなく坂、階段、小段差、壁際、移動床、ダッシュ、ノックバックを確認します。",
                "既存挙動を維持したい対象はMotor補正をOFFのままにし、1種類ずつ比較して有効範囲を広げます。",
            ...
            "共通設定はResources/json/gameplay/status_settings.jsonです。Player/EnemyのシーンJSONには個体別ステータスを保存しません。",
            {
                ...
                "Motor有効時に床を抜けず、登れない坂へ張り付かず、小段差で不自然に跳ねない。",
                "Skin幅や接地探索距離を大きくしすぎて、空中でも接地扱いになっていない。",
            },
        ...
        },
        {
            ...
            "Hierarchy最上部の「音声設定 (Audio Settings)」を選びます。",
            {
                ...
                "VFX SequencerからAudio Event JSONを指定し、実際の発生位置とCamera距離で聞こえ方を確認します。Gameplay VolumeのBGM処理はAudio EntryのIDまたはPathを指定します。",
